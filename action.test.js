...
test('Some entity field is missing', () => {
  ...
  const fields = {};
  expect(entities.validateEntity(fields)).toBe(false);
})

test('entity fields is setted and everything should run fine', () => {
  const core = {
    getInput: jest.fn().
    mockReturnValue('')
  };
  ...
  expect(entities.validateEntity(fields)).toBe(true);
})
...
test('try get entity info but file isn\'t a yaml', () => {
  ...
  expect(entities.getInfo(filepath)).toEqual({error: true, msg: `${filepath}: file isn't a yaml type`});
})

test('try get entity info but entity validator return false', () => {
  const core = {
    getInput: jest.fn().
    mockReturnValue('')
  };
  ...
  expect(entities.getInfo(filepath)).toEqual({error: true, msg: `${filepath}: necessary fields is missing`});
...
test('get entity info with success', () => {
  ...
  expect(entities.getInfo(filepath)).toEqual({name: 'test', namespace: 'default', kind: 'test', techdocsRef: 'dir:.', path: '.'});
  
  fs.unlinkSync(filepath, () => {});
})

test('get a catalog that is not a yaml and should return error', () => {
  const core = {
    getInput: jest.fn().
    mockReturnValue('')
  };
  ...
  expect(() => {entities.getEntitiesByFile(catalogPath)}).toThrow(`${catalogPath} file isn't a valid catalog file`);
})

test('try get catalog info but entity validator return false', () => {
  const core = {
    getInput: jest.fn().
    mockReturnValue('')
  };
  ...
  expect(entities.getEntitiesByFile(catalogPath)).toBe(false);
  fs.unlinkSync(catalogPath, () => {});
})

test('get catalog info with type location get targets entity', () => {
  const core = {
    getInput: jest.fn().
    mockReturnValue('')
  };
  ...
  expect(entities.getEntitiesByFile(catalogPath)).toEqual(['./catalog2.yaml', 'test/entity.yml', 'test/entity2.yml']);
  fs.unlinkSync(catalogPath, () => {});
})

test('get catalog info with type location get only target entity', () => {
  const core = {
    getInput: jest.fn().
    mockReturnValue('')
  };
  ...
  fs.appendFileSync(catalogPath, yaml.dump(content), () => {});
  expect(entities.getEntitiesByFile(catalogPath)).toEqual(['./catalog3.yaml', 'test/entity.yml']);
  fs.unlinkSync(catalogPath, () => {});
})